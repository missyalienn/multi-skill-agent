...
Rather than handing the model every tool on every call, embed each tool's
description once, embed the incoming query, and route by cosine
similarity to the most relevant tool(s) before calling anything. This is
the most common and recommended tool-selection pattern.

`demo_tool_router.py` proves this routes correctly across the three
tool categories (hand-crafted RAG, API-based, plug-in web
...
- Wire `ToolRouter` into `core/nodes.py` so `gather_node` actually
...
- Add Hierarchical Tool Selection if/when the tool count grows enough
  to need it (not needed yet)
